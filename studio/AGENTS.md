...
- Prefer `video.vir.json` as the editable source for videos built from text, panels, basic shapes, groups, and relational layout. Read `../video-ir.schema.json` and `../video-ir.example.json` before authoring it. The renderer deterministically generates `scene.py` and `narration.json`; never edit those generated files.
