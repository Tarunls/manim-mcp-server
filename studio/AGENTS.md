...
- Give long text nodes `maxWidth` and `minFontSize`; the compiler wraps using real renderer glyph metrics and fails instead of shrinking below the readability floor. Use grid groups for comparisons or repeated cards rather than hand-positioning rows.
