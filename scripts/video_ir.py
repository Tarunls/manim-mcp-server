...
SCHEMA_VERSION = "0.1"
NODE_TYPES = {"text", "panel", "rectangle", "circle", "square", "group"}
CONSTRAINT_TYPES = {"anchor", "nextTo", "align", "inside", "shift"}
CUE_ACTIONS = {"fadeIn", "create", "write", "grow", "indicate", "fadeOut"}
ENTRANCE_ACTIONS = {"fadeIn", "create", "write", "grow"}
...
def validate_video_ir(data: Any) -> VideoIRReport:
    ...
    if schema_version != SCHEMA_VERSION:
        issues.append(VideoIRIssue("$.schemaVersion", f"must be {SCHEMA_VERSION!r}."))
    ...
    if _require_keys(
        ...
        if (
            ...
            issues.append(VideoIRIssue("$.format", "Video IR v0.1 currently requires a 16:9 frame."))
    ...
    for beat_index, beat in enumerate(beats):
        ...
        allowed_beat = {
            ...
            "allowLongHold", "nodes", "constraints", "cues",
        }
        if not _require_keys(
            beat,
            beat_path,
        ...
        transition = beat.get("transitionDuration", 0.5 if beat_index < len(beats) - 1 else 0.0)
        ...
        for cue_index, cue in enumerate(cues):
            ...
            if not _require_keys(cue, cue_path, {"start", "duration", "action", "targets"}, {"start", "duration", "action", "targets"}, issues):
                continue
            start = float(cue["start"]) if _is_number(cue.get("start")) else -1.0
            cue_duration = float(cue["duration"]) if _is_number(cue.get("duration")) else 0.0
            if start < 0:
                issues.append(VideoIRIssue(f"{cue_path}.start", "must be non-negative."))
            if not 0 < cue_duration <= 10:
                issues.append(VideoIRIssue(f"{cue_path}.duration", "must be greater than zero and at most 10 seconds."))
            ...
            for target in targets:
                ...
                if action in {"indicate", "fadeOut"} and not is_visible:
...
def compile_video_ir(data: dict[str, Any]) -> CompiledVideo:
    ...
    lines = [
        ...
        f"# Video IR v{SCHEMA_VERSION}; canonical SHA-256 {vir_hash}",
        "from manim import *",
    ...
    for beat_index, beat in enumerate(data["beats"], start=1):
        lines.extend(["", f"        # Beat {beat_index}: {beat['id']} — {beat['intent']}"])
        nodes = beat["nodes"]
        ...
        for cue in beat["cues"]:
            ...
            animations = ", ".join(
                f"{action_class[cue['action']]}({_identifier(beat_index, target)})"
                for target in cue["targets"]
            )
            lines.append(f"        self.play({animations}, run_time={cue_duration:g})")
            cursor = start + cue_duration

        transition = float(beat.get("transitionDuration", 0.5 if beat_index < len(data["beats"]) else 0.0))
        active_end = float(beat["duration"]) - transition
        if active_end > cursor + 1e-9:
            lines.append(f"        self.wait({active_end - cursor:g})")
        if transition > 0:
            lines.append(f"        self.play(FadeOut({beat_group}), run_time={transition:g})")
