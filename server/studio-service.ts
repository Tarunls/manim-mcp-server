...
- Write narration.json before rendering. It must be JSON shaped as {"segments":[{"start":0.0,"text":"..."}]} with 2-8 concise segments timed to the visual beats. Keep each segment short enough to finish before the next beat. The render helper uses Speechify simba-3.2 and attaches the narration when SPEECHIFY_API_KEY is configured.
...
export class StudioService extends EventEmitter {
  ...
  private onCodexNotification(message: { method: string; params: any }) {
    ...
    if (message.method === "turn/completed") {
      ...
      if (message.params.turn.status === "completed" && fs.existsSync(output)) {
        ...
        project.status = "error";
        project.stage = "ready";
        project.error = message.params.turn.error?.message || "The agent finished without a playable render.";
      }
