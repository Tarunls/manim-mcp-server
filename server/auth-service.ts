...
export class IdentityAuthService {
  readonly cookieName =
    process.env.NODE_ENV === "production"
      ? "__Host-lesson_studio_session"
      : "lesson_studio_session";
  ...
  constructor() {
    if (getApps().length === 0) {
      const projectId =
        process.env.IDENTITY_PLATFORM_PROJECT_ID?.trim() ||
  ...
  private async identityRequest<T>(
    ...
    const response = await fetch(
      `https://identitytoolkit.googleapis.com/v1/accounts:${action}?key=${encodeURIComponent(this.apiKey)}`,
      {
        method: "POST",
    ...
    if (!response.ok)
      throw new Error(
        friendlyIdentityError(result.error?.message || "IDENTITY_ERROR"),
      );
    return result;
  }
  ...
  private userFromIdentity(result: IdentityResponse): AuthUser {
    ...
    return {
      uid: result.localId,
      email,
      emailVerified: Boolean(result.emailVerified) || isStaff,
      isStaff,
    };
  }
  ...
  async signUp(emailValue: unknown, passwordValue: unknown) {
    ...
    const result = await this.identityRequest<IdentityResponse>("signUp", {
      email,
      password,
      returnSecureToken: true,
    });
    await this.identityRequest("sendOobCode", {
      requestType: "VERIFY_EMAIL",
      idToken: result.idToken,
    });
    return {
      user: this.userFromIdentity(result),
  ...
  async signIn(emailValue: unknown, passwordValue: unknown) {
    const email = normalizedEmail(emailValue);
    const password = typeof passwordValue === "string" ? passwordValue : "";
    const result = await this.identityRequest<IdentityResponse>(
      "signInWithPassword",
      { email, password, returnSecureToken: true },
    );
    const lookup = await this.identityRequest<{ users?: IdentityResponse[] }>(
      "lookup",
      { idToken: result.idToken },
    );
    ...
    const sessionCookie = await getAuth().createSessionCookie(result.idToken, {
  ...
  async authenticate(
    ...
    try {
      const claims = await getAuth().verifySessionCookie(
        token,
