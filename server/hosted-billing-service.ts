...
export class HostedBillingService {
  ...
  get configured() {
    return Boolean(
      this.db.configured && this.stripe && process.env.STRIPE_WEBHOOK_SECRET,
    );
  }
  ...
  async getState(userId: string): Promise<BillingState> {
    ...
    return {
      ...
      entitlements: staff
        ? { ...PRICING_PLANS.pro.entitlements, creditsPerMonth: 999 }
  ...
  async assertEffort(userId: string, effort: GenerationEffort) {
    ...
    if (effortRank(effort) > effortRank(state.entitlements.maxEffort))
      throw new Error("That thinking level is available on a higher plan.");
  }

  async assertNarration(userId: string) {
    if (!(await this.getState(userId)).entitlements.narration)
      throw new Error("AI voice is available on paid plans.");
  }

  async assertLicensedAssets(userId: string) {
    if (!(await this.getState(userId)).entitlements.licensedAssets)
  ...
  async createCheckout(
    ...
    if (!prices.data[0])
      throw new Error(`Stripe price ${lookupKey} does not exist.`);
    const profile = await this.db.query<{ stripe_customer_id: string | null }>(
      "SELECT stripe_customer_id FROM billing_profiles WHERE user_id = $1",
      [userId],
    );
  ...
  async cancelForAccountDeletion(userId: string) {
    const profile = await this.db.query<{
      stripe_subscription_id: string | null;
    }>(
      "SELECT stripe_subscription_id FROM billing_profiles WHERE user_id = $1",
      [userId],
    );
