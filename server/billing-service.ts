...
export const PRICING_PLANS: Record<BillingPlanId, PricingPlan> = {
  free: {
    ...
    features: [
      "1 generation credit each month",
      "Faster thinking",
      "Precision math with polished motion",
      "Frame review and favorites",
      "MP4 download",
    ],
  },
  creator: {
  ...
  studio: {
    ...
    name: "Studio",
    monthlyPrice: 100,
    description: "Higher monthly capacity for serious publishing workflows.",
    entitlements: {
      creditsPerMonth: 70,
      maxEffort: "thorough",
      narration: true,
...
function subscriptionPeriod(subscription: Stripe.Subscription) {
  ...
  const end =
    ...
    Math.floor((Date.now() + 30 * DAY) / 1_000);
  return {
    periodStart: new Date(start * 1_000).toISOString(),
    periodEnd: new Date(end * 1_000).toISOString(),
  };
}
...
export class BillingService {
  ...
  private refreshPeriod(profile: StoredBillingProfile) {
    ...
    if (profile.plan === "free" || !ACTIVE_SUBSCRIPTIONS.has(profile.status)) {
      Object.assign(profile, freePeriod(), {
        creditsUsed: 0,
        plan: "free",
        status: "free",
      });
  ...
  getState(userId: string, email?: string): BillingState {
    ...
    if (this.staffUsers.has(userId)) {
      return {
        ...
        entitlements: {
          ...PRICING_PLANS.pro.entitlements,
          creditsPerMonth: 999,
        },
      };
    }
    const subscribed =
      profile.plan !== "free" && ACTIVE_SUBSCRIPTIONS.has(profile.status);
    ...
    return {
      ...
      creditsRemaining: Math.max(
        0,
        definition.entitlements.creditsPerMonth - profile.creditsUsed,
      ),
      periodEnd: profile.periodEnd,
      email: profile.email,
      isStaff: false,
  ...
  assertEffort(userId: string, effort: GenerationEffort) {
    ...
    if (effortRank(effort) > effortRank(state.entitlements.maxEffort)) {
      throw new Error(
        `${effort === "thorough" ? "Try harder" : "Balanced"} thinking is available on a higher plan.`,
      );
    }
  }

  assertNarration(userId: string) {
    if (!this.getState(userId).entitlements.narration)
      throw new Error("AI voice is available on paid plans.");
  }

  assertLicensedAssets(userId: string) {
    if (!this.getState(userId).entitlements.licensedAssets)
      throw new Error("Licensed visual search is available on paid plans.");
  }
  ...
  reserveGeneration(userId: string, effort: GenerationEffort) {
    ...
    if (state.creditsRemaining < cost) {
      throw new Error(
        `This request needs ${cost} generation credit${cost === 1 ? "" : "s"}. Upgrade or wait for your monthly credits to renew.`,
      );
    }
  ...
  async createCheckout(
    ...
    if (
      ...
      throw new Error(
        "Paid subscriptions are opening soon. You can use the Free plan now.",
      );
    }
    if (plan === "free")
      throw new Error("The Free plan does not need checkout.");
    ...
    if (!price)
      throw new Error(
        `Stripe price ${lookupKey} does not exist in this test account.`,
      );
    const profile = this.profile(userId, email);
  ...
  async createPortal(userId: string, baseUrl: string) {
    ...
    if (!profile.stripeCustomerId)
      throw new Error("There is no paid billing account to manage yet.");
  ...
  handleWebhook(event: Stripe.Event) {
    ...
    if (
      ...
      const profile = [...this.profiles.values()].find(
        (candidate) =>
          candidate.stripeCustomerId === customerId ||
          candidate.userId === subscription.metadata.userId,
      );
      if (!profile) return;
