...
The deployed application image and E2B template are `55fa6cf` (deployed 2026-09-12; exact E2B smoke and hosted end-to-end smoke passed; Terraform clean). Narration remains one continuous read with word timestamps, while storyboard v3 maps exact narration cues to animation actions. GPT-5.6 Sol at high reasoning writes the teaching plan and scene, diagnoses the beat-aware review frames, and repairs any critical or major issue. The renderer enforces stable text bounds and uses one continuous Manim timeline; animation-range parallelism is opt-in because it caused measurable audio/visual timing shifts. See `docs/releases/2026-09-12-video-quality.md` and `docs/VIDEO_QUALITY_ARCHITECTURE.md`.
